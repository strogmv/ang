// Code generated by ANG v0.1.127 from:
//   cue/api/auth.cue (sha: e6ddda61)
// DO NOT EDIT.

package service

import (
	"context"
	"crypto/sha256"
	"encoding/hex"
	"fmt"
	"net/http"

	"github.com/example/blog/internal/domain"
	"github.com/example/blog/internal/pkg/errors"
	"github.com/example/blog/internal/pkg/helpers"
	"github.com/example/blog/internal/port"
)

// Generated from: cue/api/auth.cue:12
func (s *AuthImpl) Register(ctx context.Context, req port.RegisterRequest) (resp port.RegisterResponse, err error) {
	existing, err := s.UserRepo.FindByEmail(ctx, req.Email)
	if err != nil {
		return resp, err
	}
	if !(existing == nil) {
		return resp, errors.New(http.StatusBadRequest, "Validation Error", "Email already registered")
	}
	var newUser domain.User
	if err := helpers.Assign(&newUser.Email, req.Email); err != nil {
		return resp, err
	}
	if err := helpers.Assign(&newUser.Name, req.Name); err != nil {
		return resp, err
	}
	_sha256Sum_5 := sha256.Sum256([]byte(req.Password))
	hash := hex.EncodeToString(_sha256Sum_5[:])
	if err := helpers.Assign(&newUser.PasswordHash, hash); err != nil {
		return resp, err
	}
	if err := helpers.Assign(&newUser.Role, "reader"); err != nil {
		return resp, err
	}
	if err := s.UserRepo.Save(ctx, &newUser); err != nil {
		return resp, err
	}
	if s.publisher != nil {
		_ = s.publisher.PublishUserRegistered(ctx, domain.UserRegistered{Email: newUser.Email, UserID: newUser.ID})
	}
	{
		_notifyMeta_10 := map[string]any{"to": fmt.Sprint(newUser.Email)}
		_notifyMsg_10 := port.NotificationMessage{Metadata: _notifyMeta_10, Template: fmt.Sprint("welcome_email"), Payload: map[string]any{"Name": newUser.Name, "LoginURL": "https://blog.ang.local/login", "AppName": "ANG Blog Example", "SupportEmail": "blog-support@ang.local"}}
		if s.dispatcher == nil {
			return resp, errors.New(http.StatusInternalServerError, "NOTIFY_DISPATCHER_NOT_CONFIGURED", "notify.Email requires notification dispatcher wiring")
		}
		if _notifyErr_10 := s.dispatcher.Dispatch(ctx, _notifyMsg_10); _notifyErr_10 != nil {
			return resp, fmt.Errorf("notify.Email: %w", _notifyErr_10)
		}
	}
	if err := helpers.Assign(&resp.ID, newUser.ID); err != nil {
		return resp, err
	}
	if err := helpers.Assign(&resp.Email, newUser.Email); err != nil {
		return resp, err
	}
	if err := helpers.Assign(&resp.Name, newUser.Name); err != nil {
		return resp, err
	}

	return resp, nil
}
