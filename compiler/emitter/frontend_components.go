...
// dedupeFormDefsByName keeps the first occurrence of each form name so manifest and forms/index.ts
// do not emit duplicate exports when the same form is derived from multiple service methods.
func dedupeFormDefsByName(defs []FormData) []FormData {
	seen := make(map[string]struct{}, len(defs))
	out := make([]FormData, 0, len(defs))
	for _, fd := range defs {
...
func (e *Emitter) EmitFrontendComponents(services []ir.Service, endpoints []ir.Endpoint, entities []ir.Entity) error {
	...
	// Collect all candidates first, deduplicate by form name, then emit — so that
	// forms/index.ts and individual schema files never contain duplicate exports.
	...
	formDefs = dedupeFormDefsByName(formDefs)
	for _, formData := range formDefs {
	...
	generatedTableFiles := make(map[string]struct{})
	for _, svc := range servicesNorm {
		for _, m := range svc.Methods {
			if !isListOperation(m.Name) || !hasFrontendListEndpoint(svc.Name, m.Name, endpointsNorm) {
				continue
			}
			...
			tableFileName := tableData.Name + "Table.tsx"
			generatedTableFiles[tableFileName] = struct{}{}
			outPath := filepath.Join("components", "tables", tableFileName)
			if err := e.emitMUITable(tableData, funcMap, outPath); err != nil {
				return err
			}
		}
	}
	// A generated output directory can outlive an operation that was removed or
	// renamed. Keep hand-authored components intact, but remove stale ANG table
	// artifacts so the barrel never imports components for missing hooks.
	if err := removeStaleGeneratedComponents(tablesDir, generatedTableFiles); err != nil {
		return err
	}
...
func hasFrontendListEndpoint(serviceName, rpc string, endpoints []normalizer.Endpoint) bool {
	for _, endpoint := range endpoints {
		if strings.EqualFold(endpoint.ServiceName, serviceName) &&
			strings.EqualFold(endpoint.RPC, rpc) &&
			strings.EqualFold(endpoint.Method, "GET") {
			return true
		}
	}
	return false
...
func (e *Emitter) emitGeneratedFormsRuntime(formDefs []FormData) error {
	...
	manifest.WriteString("import type { FormSchema } from '../ui/auto-form';\n\n")
	manifest.WriteString("export type FormSchemaLoader = () => Promise<FormSchema<Record<string, unknown>>>;\n")
	...
	renderer.WriteString("import { useEffect, useMemo, useState, type ReactNode } from 'react';\n")
	renderer.WriteString("import { AutoForm } from '../ui/auto-form';\n")
	renderer.WriteString("import type { FormSchema } from '../ui/auto-form';\n")
	...
	renderer.WriteString("  onSuccess?: (data: unknown) => void;\n")
	...
	renderer.WriteString("  fallback?: ReactNode;\n")
	...
	renderer.WriteString("  const [schema, setSchema] = useState<FormSchema<Record<string, unknown>> | null>(null);\n")
	...
	renderer.WriteString("  const formHookRegistry = FormHooks as Record<string, unknown>;\n")
	renderer.WriteString("  const mutationHookRegistry = Hooks as Record<string, unknown>;\n")
	renderer.WriteString("  const useForm = formHookRegistry[entry.useFormHook];\n")
	renderer.WriteString("  const useMutation = mutationHookRegistry[entry.useMutationHook];\n")
	...
	renderer.WriteString("  const form = (useForm as (options?: { defaultValues?: Record<string, unknown> }) => unknown)(defaultValues ? { defaultValues } : undefined);\n")
	renderer.WriteString("  const { mutate, isPending } = (useMutation as (options?: { onSuccess?: (data: unknown) => void; onError?: (error: Error) => void }) => { mutate: (data: unknown) => void; isPending?: boolean })({\n")
	renderer.WriteString("    onSuccess: (data: unknown) => onSuccess?.(data),\n")
	renderer.WriteString("    onError: (error: Error) => onError?.(error),\n")
	renderer.WriteString("  });\n")
	renderer.WriteString("  return (\n")
	renderer.WriteString("    <AutoForm\n")
	renderer.WriteString("      form={form as import('react-hook-form').UseFormReturn<Record<string, unknown>>}\n")
...
func removeStaleGeneratedComponents(dir string, expected map[string]struct{}) error {
	entries, err := os.ReadDir(dir)
	if err != nil {
		return err
	}
	...
	for _, entry := range entries {
		...
		if !strings.Contains(text, "Generated by ANG") && !strings.Contains(text, "AUTO-GENERATED by ANG") {
			continue
		}
