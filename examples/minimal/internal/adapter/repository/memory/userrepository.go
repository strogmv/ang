...
func (r *UserRepositoryStub) Insert(ctx context.Context, entity *domain.User) error {
	...
	if _, exists := r.data[entity.ID]; exists {
		return fmt.Errorf("user already exists: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}

func (r *UserRepositoryStub) Update(ctx context.Context, entity *domain.User) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if entity == nil {
		return fmt.Errorf("entity is required")
	...
	if _, exists := r.data[entity.ID]; !exists {
		return fmt.Errorf("user not found: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}
...
func (r *UserRepositoryStub) List(ctx context.Context) ([]domain.User, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	var res []domain.User
	for _, item := range r.data {
	...
	if len(res) > 1 {
		sort.Slice(res, func(i, j int) bool {
			return compareLessUser(res[i].Name, res[j].Name)
		})
	}
	return res, nil
}
