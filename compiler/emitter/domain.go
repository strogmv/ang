...
func (e *Emitter) EmitDomain(entities []ir.Entity) error {
	...
	// Keep manually maintained shared files; prune only stale entity files.
	keep["events.go"] = struct{}{}
	keep["projections.go"] = struct{}{}
	if err := pruneGeneratedFiles(
		targetDir,
		keep,
		func(name string) bool {
			return strings.HasSuffix(name, ".go") && name != "events.go" && name != "projections.go"
		},
		func(path string) bool {
			return fileContainsAny(path, "AUTO-GENERATED by ANG", "GENERATED: true")
		},
	); err != nil {
		return fmt.Errorf("prune stale domain files: %w", err)
	}
