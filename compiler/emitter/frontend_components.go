...
func (e *Emitter) emitGeneratedFormsRuntime(formDefs []FormData) error {
	...
	if len(formDefs) == 0 {
		...
		return os.WriteFile(indexPath, []byte("// Generated by ANG. Do not edit.\n"), 0644)
	}
	...
	if err := os.WriteFile(filepath.Join(formsDir, "forms.manifest.ts"), []byte(manifest.String()), 0644); err != nil {
		return err
	}
	...
	if err := os.WriteFile(filepath.Join(formsDir, "FormRenderer.tsx"), []byte(renderer.String()), 0644); err != nil {
		return err
	}
	...
	if err := os.WriteFile(filepath.Join(formsDir, "index.ts"), []byte(idx.String()), 0644); err != nil {
		return err
	}
...
`
	for _, baseDir := range paths {
		...
		if err := os.WriteFile(filepath.Join(baseDir, "index.tsx"), []byte(indexTSX), 0644); err != nil {
			return err
		}
