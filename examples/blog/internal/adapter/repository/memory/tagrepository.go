...
func (r *TagRepositoryStub) Insert(ctx context.Context, entity *domain.Tag) error {
	...
	if _, exists := r.data[entity.ID]; exists {
		return fmt.Errorf("tag already exists: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}

func (r *TagRepositoryStub) Update(ctx context.Context, entity *domain.Tag) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if entity == nil {
		return fmt.Errorf("entity is required")
	...
	if _, exists := r.data[entity.ID]; !exists {
		return fmt.Errorf("tag not found: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}
