...
func FuzzRenderFlow_InfraResilience_NoPanic(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, urlRaw, subjectRaw, keyRaw, aliasRaw string) {
		...
		steps := []normalizer.FlowStep{
			...
			{Action: "approval.Request", Args: map[string]any{"approvalKey": `"build:"+req.ID`, "title": `"Build approval"`, "requestedBy": "req.UserID", "approvers": []string{"manager@acme.io"}, "policy": `"any"`, "payload": "req", "approvalId": "approvalID"}},
			{Action: "approval.Wait", Args: map[string]any{"approvalId": "approvalID", "timeout": "100 * time.Millisecond", "onTimeout": `"reject"`, "decision": "approvalDecision"}},
			{Action: "approval.Decide", Args: map[string]any{"approvalId": "approvalID", "decision": `"approved"`, "actor": "req.UserID"}},
			{Action: "policy.Evaluate", Args: map[string]any{"policyKey": `"project.create"`, "subject": "req.UserID", "operation": `"create"`, "decision": "policyDecision", "output": "policyResult"}},
			{Action: "policy.Require", Args: map[string]any{"policyKey": `"project.create"`, "subject": "req.UserID", "operation": `"create"`}},
			{Action: "policy.Decide", Args: map[string]any{"policyKey": `"project.create"`, "subject": "req.UserID", "operation": `"create"`, "output": "policyDecisionObj"}},
			{Action: "idempotency.DeriveKey", Args: map[string]any{"from": []string{"req.UserID", "req.ID"}, "output": "idemKey"}},
			...
			{Action: "budget.Consume", Args: map[string]any{"key": "req.UserID", "tokens": "256"}},
			{Action: "context.Trim", Args: map[string]any{"input": "req.Payload", "output": "trimmedCtx", "max_bytes": 8000}},
			{Action: "profile.Require", Args: map[string]any{"key": "req.UserID", "tier": `"ops"`}},
			{Action: "concurrency.Run", Args: map[string]any{"key": `"build"`, "max": 4, "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
			{Action: "circuit.Breaker", Args: map[string]any{"name": `"external-api"`, "threshold": 3, "openTTL": "30*time.Second", "_do": []normalizer.FlowStep{{Action: "http.Call", Args: map[string]any{"method": "GET", "url": urlExpr, "output": "body2"}}}}},
			{Action: "bulkhead.Run", Args: map[string]any{"name": `"s3-upload"`, "max": 8, "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
			{Action: "log.Emit", Args: map[string]any{"level": `"info"`, "message": `"fuzz"`, "fields": map[string]string{"subject": subjectExpr}}},
			{Action: "metric.Emit", Args: map[string]any{"name": `"build.started"`, "kind": `"counter"`, "value": "1", "labels": map[string]string{"subject": subjectExpr}}},
			{Action: "trace.Span", Args: map[string]any{"name": `"FuzzSpan"`, "attrs": map[string]string{"subject": subjectExpr}, "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
			{Action: "slo.Budget", Args: map[string]any{"name": `"fuzz"`, "duration": "2*time.Second", "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
...
func FuzzRenderFlow_InfraResilience_SyntaxForSafeInputs(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, urlSeed, subjectSeed, keySeed string) {
		...
		steps := []normalizer.FlowStep{
			...
			{Action: "context.Trim", Args: map[string]any{"input": "req.Payload", "output": "trimmedCtx", "max_bytes": 8000}},
			{Action: "profile.Require", Args: map[string]any{"key": "req.UserID", "tier": `"ops"`}},
			{Action: "concurrency.Run", Args: map[string]any{"key": `"build"`, "max": maxConc, "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
			{Action: "circuit.Breaker", Args: map[string]any{"name": `"external-api"`, "threshold": 3, "openTTL": "30*time.Second", "_do": []normalizer.FlowStep{{Action: "http.Call", Args: map[string]any{"method": "GET", "url": urlExpr, "output": "body2"}}}}},
			{Action: "bulkhead.Run", Args: map[string]any{"name": `"s3-upload"`, "max": 8, "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
			{Action: "log.Emit", Args: map[string]any{"level": `"info"`, "message": `"fuzz"`}},
			{Action: "metric.Emit", Args: map[string]any{"name": `"build.started"`, "kind": `"counter"`, "value": "1"}},
			{Action: "trace.Span", Args: map[string]any{"name": `"FuzzSpan"`, "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
			{Action: "slo.Budget", Args: map[string]any{"name": `"fuzz"`, "duration": "2*time.Second", "_do": []normalizer.FlowStep{{Action: "logic.Check", Args: map[string]any{"condition": "true", "throw": "ok"}}}}},
