...
func (r *CommentRepositoryStub) Insert(ctx context.Context, entity *domain.Comment) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if entity == nil {
		return fmt.Errorf("entity is required")
	...
	if _, exists := r.data[entity.ID]; exists {
		return fmt.Errorf("comment already exists: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}

func (r *CommentRepositoryStub) Update(ctx context.Context, entity *domain.Comment) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if entity == nil {
		return fmt.Errorf("entity is required")
	...
	if _, exists := r.data[entity.ID]; !exists {
		return fmt.Errorf("comment not found: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}
func (r *CommentRepositoryStub) LockByID(ctx context.Context, id string) (*domain.Comment, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	entity, ok := r.data[id]
	if !ok {
