...
func (n *Normalizer) ExtractAuth(val cue.Value) (*AuthDef, error) {
	...
	logoutAllOp, _ := jwtVal.LookupPath(cue.ParsePath("ops.logout_all.op")).String()
	passwordChangeOp, _ := jwtVal.LookupPath(cue.ParsePath("ops.password_change.op")).String()
	...
	return &AuthDef{
		...
		LogoutAllOp:                strings.TrimSpace(logoutAllOp),
