...
type PostTagId = Types.PostTag['id'];

const postTagIdKey = (item: Types.PostTag): string =>
  String(item.id);

export interface PostTagStore {
  items: Record<string, Types.PostTag>;
  list: Types.PostTag[];
  listStale: boolean;
  detailStale: Record<string, boolean>;
  listVersion: number;
  setAll: (items: Types.PostTag[]) => void;
  upsert: (item: Types.PostTag) => void;
  remove: (id: PostTagId) => void;
  markListStale: () => void;
  markDetailStale: (id: PostTagId) => void;
  clearListStale: () => void;
  clearDetailStale: (id: PostTagId) => void;
  clear: () => void;
}
...
export const usePostTagStore = create<PostTagStore>()((set) => ({
  ...
  listVersion: 0,
  setAll: (items) => {
    const index: Record<string, Types.PostTag> = {};
    for (const item of items) {
      index[postTagIdKey(item)] = item;
    }
    set((state) => ({ items: index, list: items, listStale: false, detailStale: {}, listVersion: state.listVersion + 1 }));
  },
  upsert: (item) =>
    set((state) => {
      const key = postTagIdKey(item);
      const items = { ...state.items, [key]: item };
      const exists = state.list.some((entry) => postTagIdKey(entry) === key);
      const list = exists
        ? state.list.map((entry) => (postTagIdKey(entry) === key ? item : entry))
        : [item, ...state.list];
  ...
  remove: (id) =>
    set((state) => {
      ...
      delete items[key];
      const list = state.list.filter((entry) => postTagIdKey(entry) !== key);
...
registerStoreInvalidator('posttag', (target, params) => {
  const state = usePostTagStore.getState();
  if (target?.mode === 'detail' && target.scopeParam && params && typeof params === 'object') {
    const obj = params as Record<string, unknown>;
    const raw = obj[target.scopeParam];
    if (raw !== undefined && raw !== null && raw !== '') {
      state.markDetailStale(raw as PostTagId);
      return;
    }
  }
  state.markListStale();
