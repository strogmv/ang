...
func (e *Emitter) emitBackendEnvExample(config *normalizer.ConfigDef) error {
	if config == nil {
		config = &normalizer.ConfigDef{Fields: []normalizer.Field{}}
	}
	config = ensureRuntimeConfigFields(config)
	...
	b.WriteString("# Generated by ANG. Copy to .env and adjust values.\n")
	for _, f := range config.Fields {
		key := strings.TrimSpace(f.EnvVar)
		if key == "" {
			continue
		}
		val := strings.TrimSpace(strings.Trim(f.Default, "\""))
		b.WriteString(key)
		b.WriteString("=")
		b.WriteString(val)
		b.WriteString("\n")
...
func ensureRuntimeConfigFields(config *normalizer.ConfigDef) *normalizer.ConfigDef {
	...
	add := func(name, typ, env, def string, optional bool) {
		for _, f := range config.Fields {
	...
	add("PGMaxConnIdleTime", "string", "PG_MAX_CONN_IDLE_TIME", "30m", false)
	add("RedisAddr", "string", "REDIS_ADDR", "localhost:6379", false)
	add("NatsURL", "string", "NATS_URL", "nats://localhost:4222", false)
	add("MongoURL", "string", "MONGO_URL", "mongodb://localhost:27017", false)
	add("MongoDatabase", "string", "MONGO_DATABASE", "app", false)
	add("AWSRegion", "string", "AWS_REGION", "us-east-1", false)
	add("S3Bucket", "string", "S3_BUCKET", "", false)
	add("S3Endpoint", "string", "S3_ENDPOINT", "", false)

	// JWT defaults required by auth/http templates.
	add("JWTAlg", "string", "JWT_ALG", "HS256", false)
	add("JWTIssuer", "string", "JWT_ISSUER", "ang", false)
	...
	add("SMTPPass", "string", "SMTP_PASS", "", true)
	add("SMTPFrom", "string", "SMTP_FROM", "", true)
...
// EmitSQLCConfig writes sqlc.yaml and the db/queries directory it points at.
// Nothing in the generated project runs sqlc today, so the step is gated.
func (e *Emitter) EmitSQLCConfig() error {
	sqlcContent := `# Code generated by ANG. DO NOT EDIT.
version: "2"
sql:
...
`
	...
	return os.MkdirAll(e.outDir("db", "queries"), 0755)
}

// EmitInfraConfigs generates the Atlas config and migration helper scripts.
