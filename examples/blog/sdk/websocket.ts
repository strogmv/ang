...
export class WebSocketClient<T = WSMessage> {
  ...
  public async connect() {
    ...
    this.connectPromise = new Promise((resolve) => {
      // Token is sent as the first WS message after handshake — never in the URL.
      // URL-based tokens appear in server access logs, browser history, and Referer headers.
      this.ws = new WebSocket(this.url);

      this.ws.onopen = () => {
        const activeToken = this.token || useAuthStore.getState().token;
        if (activeToken) {
          this.ws!.send(JSON.stringify({ type: 'auth', token: activeToken }));
        }
