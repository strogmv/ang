...
func (r *PostRepositoryStub) Insert(ctx context.Context, entity *domain.Post) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if entity == nil {
		return fmt.Errorf("entity is required")
	...
	if _, exists := r.data[entity.ID]; exists {
		return fmt.Errorf("post already exists: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}

func (r *PostRepositoryStub) Update(ctx context.Context, entity *domain.Post) error {
	r.mu.Lock()
	defer r.mu.Unlock()
	if entity == nil {
		return fmt.Errorf("entity is required")
	...
	if _, exists := r.data[entity.ID]; !exists {
		return fmt.Errorf("post not found: %s", entity.ID)
	}
	r.data[entity.ID] = entity
	return nil
}
func (r *PostRepositoryStub) LockByID(ctx context.Context, id string) (*domain.Post, error) {
	r.mu.RLock()
	defer r.mu.RUnlock()
	entity, ok := r.data[id]
	if !ok {
